...
import { buildHiddenColumnSet, isColumnHidden, normalizeColumnLabel } from '../utils/exportColumns.js'
  ...
  type StudentSummaryCache = {
    ...
    maxRawMarksByAssessment: Record<string, number | null>;
  }
  ...
  let { 
	...
	const gradeBands = [...DEFAULT_GRADE_RANGES].reverse().map(range => ({
		grade: range.grade,
		rangeLabel: `${range.lowerPercentage}-${range.upperPercentage === 100 ? '' : '<'}${range.upperPercentage}%`
	}));
  ...
  $effect(() => {
	...
	function getMaxPossibleRawMarks(assessmentId: string): number | null {
		return studentSummaryCache.maxRawMarksByAssessment?.[assessmentId] ?? null;
	}
	...
	function buildStudentSummaryCache(): StudentSummaryCache {
		...
		for (const student of students) {
			...
			for (const assessment of markingAssessments) {
				...
				const marksResult = evaluation
					? { ...getMarkSummary(assessment, evaluation.categoryMarks || {}), categoryMarks: evaluation.categoryMarks || {} }
					: null;

				marksByAssessment[assessmentId] = marksResult;

				let weightedResult = null;
				if (marksResult?.isComplete && marksResult.percentage !== null && Number.isFinite(assessment.weight) && assessment.weight > 0) {
					const weightedMarks = (marksResult.percentage / 100) * assessment.weight;
					weightedResult = {
			...
			const percentage = isComplete ? (totalWeightedMarks / totalWeight) * 100 : null;
	...
	function handlePrintToDownload() {
		...
		const allPossibleGrades = gradeBands.map(band => band.grade);
		
		// Grade ranges mapping
		const gradeRanges = Object.fromEntries(gradeBands.map(band => [band.grade, band.rangeLabel]));
	...
	function getPerformanceHighlights() {
		...
		}).filter(perf => perf.finalGrade !== "N/A").sort((a, b) => b.percentage - a.percentage);
	...
	function getAssessmentPerformanceHighlights(assessmentId: string) {
		...
		const studentsWithAssessmentMarks = studentsWithMarks.filter(student => {
			...
			return marks && marks.isComplete && marks.percentage !== null;
		});
		...
		const studentPerformances = studentsWithAssessmentMarks.map(student => {
			...
			const percentage = marks.percentage;
			const grade = getGradeInfo(percentage, assessment.percentageRanges || []).grade;
	...
	function getGradeDistribution() {
		...
		return gradeBands.map(band => ({
			grade: band.grade,
			label: `${band.grade} (${band.rangeLabel})`,
			count: gradeCounts[band.grade] || 0,
			color: getGradeColor(band.grade)
		}));
	}
...
<div class="container-fluid assessment-manager">
...
{#each assessments as assessment (assessment.id)}
          ...
          <button 
													...
													<div class="d-flex align-items-center">
														...
														<div class="flex-grow-1">
															<div class="d-flex align-items-center justify-content-between">
																...
																<div class="d-flex gap-1">
																	<button 
																		class="btn btn-sm btn-outline-secondary ui-icon-action"
																	...
																	<button 
																		class="btn btn-sm btn-outline-danger ui-icon-action"
								...
								{#each assessments as assessment (assessment.id)}
									...
									{@const barPercentage = marks?.percentage ?? 0}
									<!-- Marks Column -->
											...
											{#if weighted}
																	...
																	<div class="progress" style="height: 8px; background-color: #e9ecef;">
																		<div 
																			class="progress-bar" 
																			role="progressbar" 
																			style="width: {Math.min(Math.max(barPercentage, 0), 100)}%; background-color: {getDynamicColor(marks.total, maxRawMarks)};"
																			aria-valuenow={marks.total} 
																			aria-valuemin="0" 
																			aria-valuemax={maxRawMarks}
																		></div>
      </div>
