...
test('fills category maxima during setup and shows numbered actions when a student is loaded', async ({ page }) => {
  await openRubric(page, [{ id: 'project', name: 'Project', allocatedMarks: 10 }, { id: 'other', name: 'Other', allocatedMarks: 5 }])
  await expect(page.getByText('(30 allocated)', { exact: true })).toBeVisible()
  const fill = page.getByRole('button', { name: 'Fill category marks from table', exact: true })
  await fill.click()
  await expect(page.locator('#total-marks-input')).toHaveValue('25')
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem('feedback-subjects')).subjects[0].assessments[0].categories.map(category => category.allocatedMarks))).toEqual([25, 5])
  await expect(page.getByRole('alert').filter({ hasText: 'Check allocation' })).toBeVisible()
  await captures(page, fill.locator('..'), 'fill-maxima')
  await page.getByRole('button', { name: 'Student:', exact: true }).click()
  await page.getByRole('button', { name: /Synthetic Student/ }).click()
  await expect(fill).toHaveCount(0)
  const assign = page.getByRole('button', { name: 'Assign marks to all headings', exact: true })
  await expect(assign).toHaveText('1. Assign marks to all headings')
  await expect(page.getByRole('button', { name: '2. Improve all with RAG', exact: true })).toBeVisible()
  await captures(page, assign.locator('../..'), 'student-actions')
  await page.reload()
  await page.getByRole('button', { name: 'Open subject Rubric Marks QA', exact: true }).click()
  await page.getByRole('button', { name: /Open Feedback/ }).first().click()
  await expect(page.getByText('(30 allocated)', { exact: true })).toBeVisible()
