...
test('fills configured category maxima from the table without changing student marks or total', async ({ page }) => {
  ...
  const mark = page.getByRole('spinbutton', { name: 'Marks for Project', exact: true })
  await mark.fill('20')
  await page.getByRole('button', { name: 'Fill category marks from table', exact: true }).click()
  await expect(mark).toHaveValue('20')
  ...
  const toolbar = page.getByRole('button', { name: 'Fill category marks from table', exact: true }).locator('..')
  await captures(page, toolbar, 'fill-maxima')
  await page.reload()
  await page.getByRole('button', { name: 'Open subject Rubric Marks QA', exact: true }).click()
  await page.getByRole('button', { name: /Open Feedback/ }).first().click()
  await expect(page.getByText('(30 allocated)', { exact: true })).toBeVisible()
