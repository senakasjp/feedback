...
- **Never let `shortFeedback` bias retrieval**: "Improve with RAG" writes its own output back into the same draft textarea it reads from (`quickAddText`). If a re-run's search query includes that draft text, it creates a feedback loop — a wrong prior answer's vocabulary pulls in unrelated evidence on the next run, reinforcing the mistake. `buildQueryText()` (the rubric/reference-material retrieval query — the only retrieval left, now that the student submission itself is always sent in full) deliberately excludes `shortFeedback`; it's shown to the model as context only, never used for search.
- **Student submission always goes in full, never chunked/excerpted**: an earlier RAG pipeline chunked the submission by heading, ran hybrid lexical+embedding retrieval per rubric criterion, and sent only the top-matching excerpt to the model. It was deliberately removed (`e5ad48f`) — narrowing the submission before the AI sees it silently drops anything that doesn't score well by keyword/semantic similarity to a category (a bibliography, a claim in an earlier/later chapter, evidence not phrased like the category name). Improve with RAG, Evidence Check, and AI Draft all now send the student's full submission text for every category, same as each other — don't reintroduce per-category excerpting or a student-submission embedding index. The assessment/rubric-context RAG (`buildAssessmentRagContext`, the per-assessment `vectorIndex`) is unrelated and untouched — that's ranking rubric/prior-feedback chunks, not narrowing the student's own answer. Check: `tests/aiMarkingService.fullSubmissionText.check.mjs`.
