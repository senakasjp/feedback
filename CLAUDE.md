...
- **Never let `shortFeedback` bias retrieval**: "Improve with RAG" writes its own output back into the same draft textarea it reads from (`quickAddText`). If a re-run's search query includes that draft text, it creates a feedback loop — a wrong prior answer's vocabulary pulls in unrelated evidence on the next run, reinforcing the mistake. `buildCriterionSubmissionQuery()` and the RAG context retrieval query deliberately exclude `shortFeedback`; it's shown to the model as context only, never used for search.
- **Student-submission RAG pipeline** (`aiMarkingService.js`): `buildAssessmentRagContext()` chunks the raw submission by heading with overlap (`buildStudentSubmissionChunks`), retrieves per rubric criterion via hybrid lexical+embedding scoring, expands the top match with its neighbouring chunks (`expandWithNeighbors`), and retries once with a broadened query if the best match is weak (`retrieveSubmissionEvidence`'s sufficiency check). Headings/ToC lines are excluded from being scored as evidence (they trivially keyword-match a category's own name with no real content) — a category name matching only a heading, not body text, correctly stays "insufficient" rather than citing the heading as evidence. Criteria with no reliable evidence are surfaced via `insufficientCriteria`/`retrievalSufficient` and threaded into the prompt so the model says so explicitly instead of guessing. Optional `studentSubmissionIndex` param mirrors the existing `vectorIndex` caching pattern but isn't wired up from `App.svelte` yet — embeddings are recomputed per marking call (one batched request covering all criteria) until that's added. Self-check: `src/services/aiMarkingService.selfcheck.js`.
